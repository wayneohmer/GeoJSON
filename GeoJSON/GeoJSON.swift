...
public struct GeoJSONkeys {
    ...
    public static let multiPoint = "MultiPoint"
    public static let lineString = "LineString"
    ...
    public static let featureCollection = "FeatureCollection"
    public static let id = "id"
    public static let bbox = "bbox"
    public static let type = "type"
    public static let crs = "crs"
    ...
    public class GeoJSONGeometry: NSObject{
        ...
        public func surroundsPoint(testPoint:CLLocation) -> Bool{
            ...
            func evaluatePolygon(polygon:[[CLLocation]]) -> Bool{
                ...
                // See if point is in the bounding box. Fast way to get a false.
                if (testPoint.coordinate.latitude <= self.boundsNorthEast.latitude && testPoint.coordinate.latitude >= self.boundsSouthWest.latitude &&
                    testPoint.coordinate.longitude >= self.boundsSouthWest.longitude && testPoint.coordinate.longitude <= self.boundsNorthEast.longitude){
                        
                        // We know is it inside the bounding box, now do the expensive calculation.
